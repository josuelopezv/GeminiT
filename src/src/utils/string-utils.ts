...
// logger.info('StringUtils module loaded and logger initialized.'); // This can be noisy, let's comment it for now
...
export function stripAnsiCodes(str: string): string {
    ...
    for (let i = 0; i < cleanedStr.length; i++) {
        ...
        // Ensure this detailed log is active
        ...
        if (charCode === 8) { // BS (Backspace, char code 8)
            ...
            logger.debug(`[stripAnsiCodes DETAIL] Discarding other control char: ${JSON.stringify(char)}, Code: ${charCode}`);
        } else {
            result += char;
        }
    }
