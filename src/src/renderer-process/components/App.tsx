...
const App: React.FC = () => {
    ...
    const [isSettingsPanelVisible, setIsSettingsPanelVisible] = useState(false);    const [apiKey, setApiKey] = useState<string>('');
    const [modelName, setModelName] = useState<string>('');
    const [modelInstruction, setModelInstruction] = useState<string>('');
    ...
    }, [initialWidthSet]); // Runs when initialWidthSet changes, or on mount    // Effect to load initial settings from electron-store
    useEffect(() => {
        ...
        ipcRenderer.invoke('settings:get-initial-model-instruction').then(savedInstruction => {
            if (savedInstruction) setModelInstruction(savedInstruction);
        });
    }, []); // Runs once on mount
    ...
    const handleModelInstructionChange = useCallback((newInstruction: string) => {
        setModelInstruction(newInstruction);
    }, []);
    ...
    return (
        ...
        <div ref={appContainerRef} className="flex h-screen font-sans overflow-hidden">
            ...
            >
                ...
                />
            </div>            {isSettingsPanelVisible && (
                <SettingsPanelComponent 
                    isVisible={isSettingsPanelVisible} 
                    onClose={() => setIsSettingsPanelVisible(false)}
                    initialApiKey={apiKey}
                    initialModelName={modelName}
                    initialModelInstruction={modelInstruction}
                    onApiKeyChange={handleApiKeyChange}
                    onModelNameChange={handleModelNameChange}
                    onInitialModelInstructionChange={handleModelInstructionChange}
                />
            )}
        </div>
    );
